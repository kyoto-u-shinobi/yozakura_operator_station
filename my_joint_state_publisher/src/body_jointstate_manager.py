...
class BodyJointStateManager:
    ...
    def __base_callback(self, base_state):
        if base_state.is_ok:
            ...
            self.base_jointstate.position.append(np.radians(base_state.body_front.pitch.data))
            self.base_jointstate.position.append(np.radians(base_state.body_front.roll.data))
            self.base_jointstate.position.append(np.radians(base_state.body_back.pitch.data))
            self.base_jointstate.position.append(np.radians(base_state.body_back.roll.data))
            self.base_jointstate.position.append(np.radians(base_state.flipper_left.data))
            self.base_jointstate.position.append(np.radians(base_state.flipper_right.data))
            self.base_jointstate.position.append(np.radians(base_state.wheel_left.data))
            self.base_jointstate.position.append(np.radians(base_state.wheel_right.data))
