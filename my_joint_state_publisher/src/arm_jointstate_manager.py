...
class ArmJointStateManager:
    ...
    def __init__(self):
        ...
        self.arm_jointstate.position = [np.radians(0.0)] * len(self.arm_jointstate.name)
    ...
    def __arm_callback(self, arm_state):
        if arm_state.is_ok:
            jack_base_angle = np.radians((360.0 - 2.0 * float(arm_state.top_angle.data)) / 2.0)
            self.arm_jointstate.position = []
            self.arm_jointstate.position.append(np.radians(arm_state.yaw.data))
            self.arm_jointstate.position.append(np.radians(arm_state.pitch.data))
